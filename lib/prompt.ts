...
5. 근거 없이 "당신은 ~이다"라고 과도하게 확정하지 않는다. 텍스트에서
  관찰되는 패턴(문장 길이, 면���˃�/ㅋㅋ 사용,
   질문 빈도, 감탄사, 말줄임표, 존댓말/반말 전환 등)에 근거를 두고,
   근거가 약하면 단정적 어조를 낮춘다.
6. 공격적이거나 모욕적인 표현, 외모/능력/가치를 깎아내리는 표현한다.
...
// 텍스트 대신(또는 텍스트와 함께) 스크린샷 이미지가 입력될 때 추가되는 지시문.
const IMAGE_INPUT_SECTION = `
# 이미지로 입력된 경우
입력이 텍스트가 아니라 사용자가 캡처한 스크린샷 이미지로 주어질 수도 있다.
- 이미지 속 대화창/게시글의 실제 텍스트(말풍선 안 내용, 글 본문)만 분석
  대상으로 삼는다. 상태바, 시간 표시, 프로필 사진, 읽음 표시, 앱 UI
  아이콘 등은 분석 대상이 아니다.
- 카카오톡 등 메신저 스크린샷에서는 보통 사용자 자신이 보낸 메시지가
  화면 오른쪽에 노란색 등 구분되는 색상의 말풍선으로 표시되고, 상대방
  메시지는 왼쪽에 다른 색(흰색 등)으로 표시된다. 분석 대상이 "(오른쪽
  말풍선)"처럼 위치/색상으로 지정되어 있다면, 그 시각적 특징에 해당하는
  말풍선의 텍스트만 분석�대상으로 삼는다.
- 이미지가 여러 장 주어지면 순서대로 이어지는 하나의 대화/글로 간주한다.
- 이미지 속 글자를 읽기 어려운 부분이 있다면 과도하게 추측하지 말고,
  확실히 읽히는 부분 위주로 조심스럽게 분석한다.`;
...
# 이미지로 입력된 경우
텍스트 대신 스크린샷 이미지가 주어질 수도 있다.
- 메신저 대화 스크린샷이면: 화면에 이름이 텍스트로 표시된 사람은 그
  이름을 그대로 speakers에 넣는다 (보통 그룹 채팅에서 상대방 이름 위에
  표시됨).
- 사용자 자신이 보낸 메시지는 이름이 따로 안 붙어 있어도, 화면 오른쪽에
  노란색 등 구분되는 색상의 말풍선으로 표시되는 경우가 많다. 이런
  경우엔 speakers에 "나 (오른쪽 말풍선)"처럼 위치/색상으로 설명하는
  라벨을 하나 포함시킨다. 왼쪽 말풍선의 상대방 이름을 알 수 없으면
  "상대방 (왼쪽 말풍선)"처럼 라벨링해도 된다.
- 이미지가 대화 스크린샷이 아니라 일기/메모/SNS 게시글을 캡처한
  것이면 isDialogue는 false로 한다.
- 이미지가 여러 장이면 하나의 이어지는 대화/글로 간주해서 판별한다.
